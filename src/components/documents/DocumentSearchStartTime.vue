                ...
                <v-expansion-panel-content :class="`${hover ? 'primary' : ''}`" ripple>
                    ...
                    <template #header>
                        <span :style="`color: ${hover ? 'white' : ''}`">
                            ...
                            Tìm theo thời gian tài liệu bắt đầu
                        </span>
                    </template>
    ...
    export default {
        ...
        methods: {
            reset() {
                ...
                this.$store.commit('DOCUMENT_STORE/SET_START_TIME_FROM_SEARCH_VALUE', null);
                this.$store.commit('DOCUMENT_STORE/SET_START_TIME_TO_SEARCH_VALUE', null);
            },
            clearStartDateFrom() {
                this.startTimeFrom = null;
                this.startDateFrom = null;
                this.$store.commit('DOCUMENT_STORE/SET_START_TIME_FROM_SEARCH_VALUE', this.startTimeFromComputed);
            },
            saveStartDateFrom() {
            ...
            saveStartTimeFrom() {
                ...
                if (this.startDateFrom === this.startDateTo) {
                    this.$store.commit('DOCUMENT_STORE/SET_START_TIME_TO_SEARCH_VALUE', this.startTimeToComputed);
                }
                this.$store.commit('DOCUMENT_STORE/SET_START_TIME_FROM_SEARCH_VALUE', this.startTimeFromComputed);
            },
            clearStartDateTo() {
